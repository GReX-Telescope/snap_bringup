from casperfpga.synth import *
from casperfpga.adc import *
from casperfpga.clockswitch import *
from casperfpga.wishbonedevice import *
import logging
import numpy as np
import time
...
class SNAPADC(object):
    ...
    def bitslip(self, chipSel=None, laneSel=None):
        ...
        self.logger.debug(
            "Bitslip lane {0} of chip {1}".format(str(laneSel), str(chipSel))
        )
        ...
        if mode == "lane_wise_single_pat":
            ...
            for adc in self.adcList:
                for lane in self.laneList:
                    ...
                    if not t:
                        self.logger.error(
                            "ADC{0} lane{1} delay decision failed".format(adc, lane)
                        )
                    else:
            ...
            errs = self.testPatterns(
                taps=True, mode="std", pattern1=self.p1, pattern2=self.p2
            )
